...
function test_all_ini_entries()
{
	...
	$helptext = array(
		...
		'opcache.jit' => "The JIT is enabled, introducing a RWX memory segment in PHP processes, which might facilitate memory-corruption based attacks",
	...
	foreach (ini_get_all() as $k => $v) {
		...
		case 'opcache.jit':
			if (intval($v) & 1 != 0 ) {
				list($result, $reason) = array(TEST_LOW, "JIT enabled.");
			}
			break;
